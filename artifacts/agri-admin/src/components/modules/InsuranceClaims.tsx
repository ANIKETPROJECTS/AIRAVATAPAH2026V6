...
import { ChevronLeft, ChevronRight, Search, RefreshCw, Shield, ArrowLeft, AlertTriangle, Trash2, Pencil, ChevronDown, X } from "lucide-react";
  ...
  return (
    <div className="relative">
      <select
        ...
        className="appearance-none pl-3 pr-8 py-2 text-sm border border-black/15 rounded-full bg-white text-black focus:outline-none focus:ring-2 focus:ring-blue-300 cursor-pointer font-medium"
      >
...
export default function InsuranceClaims() {
  ...
  const cropOptions = useMemo(() => {
    const crops = [...new Set(apps.map(a => a.crop).filter(Boolean))] as string[];
    return crops.map(c => ({ label: c, value: c }));
  }, [apps]);

  const insuranceOptions = useMemo(() => {
    const names = [...new Set(apps.map(a => a.schemeName).filter(Boolean))];
    return names.map(n => ({ label: n, value: n }));
  }, [apps]);

  const filtered = useMemo(() => {
    let list = tab === "all" ? apps : apps.filter(a => a.status === tab);
    if (filterCrop) list = list.filter(a => a.crop === filterCrop);
    if (filterPriority) list = list.filter(a => getPriority(a.appliedAt).label === filterPriority);
    if (filterInsurance) list = list.filter(a => a.schemeName === filterInsurance);
    if (search.trim()) {
    ...
    return list;
  }, [apps, tab, search, filterCrop, filterPriority, filterInsurance]);
  ...
  const activeFilterCount = [filterCrop, filterPriority, filterInsurance].filter(Boolean).length;
  ...
  async function updateStatus(id: string, status: string, adminNotes?: string) {
    ...
    try {
      ...
      const msgs: Record<string, string> = { Approved: `Claim ${id} approved`, Settled: `Claim ${id} settled`, Rejected: `Claim ${id} rejected`, "Under Review": `Claim ${id} under review` };
          ...
          <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div><h3 className="font-semibold text-base">Reject Claim</h3><p className="text-xs text-gray-500">{rejectModal.id}</p></div>
              </div>
              <div>
                ...
                <textarea value={rejectReason} onChange={e => setRejectReason(e.target.value)}
                  ...
                  placeholder="Enter the reason for rejecting this claim…" autoFocus/>
              </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
          ...
          <div className="bg-gradient-to-r from-blue-700 to-blue-900 px-6 py-5">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-blue-200 text-xs font-medium uppercase tracking-wider mb-1">Insurance Claim Review</p>
                <h2 className="text-white font-bold text-xl">{review.schemeName}</h2>
                <p className="text-blue-300 text-xs font-mono mt-1">{review.applicationId}</p>
              </div>
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="border border-gray-100 rounded-xl overflow-hidden">
              <div className="bg-gray-50 px-4 py-2.5 border-b border-gray-100">
                <h3 className="text-xs font-bold text-gray-600 uppercase tracking-wider">Claim Details</h3>
              </div>
              <div className="p-4 grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div className="col-span-2"><p className="text-[11px] text-gray-400 font-medium uppercase tracking-wide">Insurance Scheme</p><p className="font-semibold text-gray-800 mt-0.5">{review.schemeName}</p></div>
                <div><p className="text-[11px] text-gray-400 font-medium uppercase tracking-wide">Crop</p><p className="text-gray-700 mt-0.5">{review.crop ?? "—"}</p></div>
                <div><p className="text-[11px] text-gray-400 font-medium uppercase tracking-wide">Land (Acres)</p><p className="text-gray-700 mt-0.5">{review.land ?? "—"}</p></div>
                <div><p className="text-[11px] text-gray-400 font-medium uppercase tracking-wide">Filed</p><p className="text-gray-700 mt-0.5">{formatDate(review.appliedAt)}</p></div>
                <div><p className="text-[11px] text-gray-400 font-medium uppercase tracking-wide">Age</p><p className="text-gray-700 mt-0.5">{timeAgo(review.appliedAt)}</p></div>
            ...
            {review.lossDescription && (
              <div className="border border-red-100 rounded-xl overflow-hidden">
                <div className="bg-red-50 px-4 py-2.5 border-b border-red-100">
                  <h3 className="text-xs font-bold text-red-600 uppercase tracking-wider">Loss Description</h3>
                </div>
                <div className="p-4 bg-red-50/40">
                  <p className="text-sm text-red-900 leading-relaxed">{review.lossDescription}</p>
                </div>
              </div>
            )}
              ...
              <div className="border border-gray-100 rounded-xl overflow-hidden">
                ...
                <div className="p-4 flex flex-wrap gap-2">
                  {review.documentRefs.map(ref => (
                    <span key={ref} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-blue-50 border border-blue-200 text-blue-700 text-xs font-semibold">
            ...
            <div className="border border-gray-100 rounded-xl overflow-hidden">
              ...
              <div className="p-4">
                <textarea value={notes} onChange={e => setNotes(e.target.value)}
                  className="w-full px-4 py-3 text-sm bg-white border border-gray-200 rounded-xl h-28 resize-none focus:outline-none focus:ring-2 focus:ring-blue-300"
            ...
            <div className="flex gap-3 flex-wrap pt-1 border-t border-gray-100">
              ...
              {review.status === "Approved" && (
                <button disabled={saving} onClick={() => updateStatus(review.applicationId, "Settled", notes)}
                  className="text-sm px-6 py-2.5 rounded-full bg-teal-600 text-white hover:bg-teal-700 disabled:opacity-60 font-semibold transition-colors">
                  Settle Claim
                </button>
              )}
    ...
    <div className="space-y-5" style={{ fontFamily: "Poppins, sans-serif" }}>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div><h3 className="font-semibold text-base">Delete Claim</h3><p className="text-xs text-gray-500 font-mono">{deleteModal.id}</p></div>
            </div>
            <p className="text-sm text-gray-500">Permanently delete the claim for <span className="font-semibold text-gray-800">"{deleteModal.name}"</span>? This cannot be undone.</p>
            <div className="flex gap-2 justify-end">
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div><h3 className="font-semibold text-base">Edit Claim</h3><p className="text-xs text-gray-500 font-mono">{editModal.applicationId}</p></div>
            </div>
            <div className="space-y-3">
              <div>
                ...
                <textarea value={editReply} onChange={e => setEditReply(e.target.value)}
                  className="w-full px-3 py-2 text-sm border border-gray-200 rounded-xl h-20 resize-none focus:outline-none focus:ring-2 focus:ring-blue-200"
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button disabled={saving} onClick={saveEdit} className="px-5 py-2 text-sm rounded-full bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 font-medium">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-6 gap-3">
        {[
          ...
          { label: "Settled",      value: counts.settled,  bg: "bg-teal-600",    key: "Settled" },
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="pl-9 pr-4 py-2 text-sm border border-black/15 rounded-full w-64 focus:outline-none focus:ring-2 focus:ring-blue-300 bg-white text-black"
          />
          {search && (
        ...
        <FilterSelect
          ...
          onChange={v => { setFilterInsurance(v); setPage(0); }}
        ...
        {filterCrop && (
          <span className="flex items-center gap-1.5 text-xs font-medium text-white bg-amber-500 rounded-full px-3 py-1.5">
            Crop: {filterCrop}
            <button onClick={() => setFilterCrop("")} className="hover:text-white/70"><X className="h-3 w-3"/></button>
          </span>
        )}
        {filterPriority && (
        ...
        {filterInsurance && (
          <span className="flex items-center gap-1.5 text-xs font-medium text-white bg-blue-600 rounded-full px-3 py-1.5 max-w-[180px]">
            <span className="truncate">{filterInsurance}</span>
            <button onClick={() => setFilterInsurance("")} className="hover:text-white/70 shrink-0"><X className="h-3 w-3"/></button>
          </span>
        )}
        {activeFilterCount > 0 && (
          <button onClick={() => { setFilterCrop(""); setFilterPriority(""); setFilterInsurance(""); setPage(0); }}
      ...
      <div className="bg-white border border-black/10 rounded-xl overflow-hidden shadow-sm">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-2">
            ...
            <p className="text-sm text-black/50">No insurance claims found.</p>
          </div>
        ) : (
          <>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead><tr className="bg-white border-b border-black/8 text-left">
                  ...
                  <th className="px-4 py-3 text-sm font-semibold text-black">Insurance</th>
                  <th className="px-4 py-3 text-sm font-semibold text-black">Crop</th>
                  <th className="px-4 py-3 text-sm font-semibold text-black">Land (Acres)</th>
                  <th className="px-4 py-3 text-sm font-semibold text-black">Date Filed</th>
