...
import { ChevronLeft, ChevronRight, Search, RefreshCw, FileText, ArrowLeft, AlertTriangle, Trash2, Pencil, ChevronDown, X } from "lucide-react";
...
interface FarmerDetail {
  ...
  addedAt?: string; surveyNumber?: string;
}
  ...
  return (
    <div className="relative">
      <select
        ...
        className="appearance-none pl-3 pr-8 py-2 text-sm border border-black/15 rounded-full bg-white text-black focus:outline-none focus:ring-2 focus:ring-emerald-300 cursor-pointer font-medium"
      >
...
export default function SchemeApplications() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filterScheme) list = list.filter(a => a.schemeName === filterScheme);
    if (search.trim()) {
    ...
    return list;
  }, [apps, tab, search, filterType, filterPriority, filterScheme]);
  ...
  const activeFilterCount = [filterType, filterPriority, filterScheme].filter(Boolean).length;
  ...
  async function updateStatus(id: string, status: string, adminNotes?: string) {
    ...
    try {
      ...
      showToast(status === "Approved" ? `Application ${id} approved` : status === "Rejected" ? `Application ${id} rejected` : `${id} moved to Under Review`);
  ...
  if (review) {
    return (
      <div className="space-y-4" style={{ fontFamily: "Poppins, sans-serif" }}>
        {toast && <div className="fixed top-4 right-4 z-50 bg-gray-900 text-white px-4 py-3 rounded-full shadow-lg text-sm font-medium">{toast}</div>}
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 font-mono">{rejectModal.id}</p>
                </div>
              </div>
              <div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
          ...
          <div className="bg-gradient-to-r from-emerald-700 to-emerald-900 px-6 py-5">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-emerald-200 text-xs font-medium uppercase tracking-wider mb-1">Scheme Application Review</p>
                <h2 className="text-white font-bold text-xl">{review.schemeName}</h2>
                <p className="text-emerald-300 text-xs font-mono mt-1">{review.applicationId}</p>
              </div>
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="border border-gray-100 rounded-xl overflow-hidden">
              <div className="bg-gray-50 px-4 py-2.5 border-b border-gray-100">
                <h3 className="text-xs font-bold text-gray-600 uppercase tracking-wider">Scheme Details</h3>
              </div>
              <div className="p-4 grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div className="col-span-2"><p className="text-[11px] text-gray-400 font-medium uppercase tracking-wide">Scheme Name</p><p className="font-semibold text-gray-800 mt-0.5">{review.schemeName}</p></div>
                <div>
                  <p className="text-[11px] text-gray-400 font-medium uppercase tracking-wide">Type</p>
                  {review.schemeType ? (
                    <span className="text-xs px-2.5 py-0.5 rounded-full bg-blue-600 text-white font-medium mt-0.5 inline-block">{review.schemeType}</span>
              ...
              <div className="border border-gray-100 rounded-xl overflow-hidden">
                ...
                <div className="p-4 flex flex-wrap gap-2">
                  {review.documentRefs.map(ref => (
                    <span key={ref} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs font-semibold">
            ...
            <div className="border border-gray-100 rounded-xl overflow-hidden">
              ...
              <div className="p-4">
                <textarea value={notes} onChange={e => setNotes(e.target.value)}
                  className="w-full px-4 py-3 text-sm bg-white border border-gray-200 rounded-xl h-28 resize-none focus:outline-none focus:ring-2 focus:ring-emerald-300"
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button disabled={saving} onClick={saveEdit} className="px-5 py-2 text-sm rounded-full bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-50 font-medium">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="pl-9 pr-4 py-2 text-sm border border-black/15 rounded-full w-64 focus:outline-none focus:ring-2 focus:ring-emerald-300 bg-white text-black"
          />
          {search && (
        ...
        <FilterSelect
          label="Scheme Name"
          value={filterScheme}
          onChange={v => { setFilterScheme(v); setPage(0); }}
        ...
        {filterScheme && (
          <span className="flex items-center gap-1.5 text-xs font-medium text-white bg-emerald-600 rounded-full px-3 py-1.5 max-w-[180px]">
            <span className="truncate">{filterScheme}</span>
            <button onClick={() => setFilterScheme("")} className="hover:text-white/70 shrink-0"><X className="h-3 w-3"/></button>
          </span>
        )}
        {activeFilterCount > 0 && (
          <button onClick={() => { setFilterType(""); setFilterPriority(""); setFilterScheme(""); setPage(0); }}
      ...
      <div className="bg-white border border-black/10 rounded-xl overflow-hidden shadow-sm">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-2">
            ...
            <p className="text-sm text-black/50">No scheme applications found.</p>
          </div>
        ) : (
          <>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead><tr className="bg-white border-b border-black/8 text-left">
                  ...
                  <th className="px-4 py-3 text-sm font-semibold text-black">Scheme</th>
